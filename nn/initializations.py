...
def lecun_uniform(shape, name=None):
    ...
    return uniform(shape, scale, name=None)
...
def he_normal(shape, name=None):
    ...
    fan_in, fan_out = get_fans(shape)
    s = np.sqrt(2. / fan_in)
    return normal(shape, s, name=None)


def he_uniform(shape, name=None):
    fan_in, fan_out = get_fans(shape)
    s = np.sqrt(6. / fan_in)
    return uniform(shape, s, name=None)
...
def orthogonal(shape, scale=1.1, name=None):
    ...
    q = q.reshape(shape)
    return sharedX(scale * q[:shape[0], :shape[1]], name=None)
...
def identity(shape, scale=1, name=None):
    if len(shape) != 2 or shape[0] != shape[1]:
        ...
        return sharedX(scale * np.identity(shape[0]), name=None)
